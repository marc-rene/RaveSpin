...
func _ready() -> void:
    ...
    _jump_beat_jump_btn.toggled.connect(_on_jump_beat_jump_toggled)
    _beat_jump_toggle_btn.toggled.connect(_on_beat_jump_toggle_toggled)
...
func _sync_ui_from_controller() -> void:
    ...
    _beat_jump_toggle_btn.button_pressed = true

    _hot_cue_mode_btn.disabled = mode != DJ_Controller.E_Performance_Pad_Mode.HOT_CUE
    _beat_jump_toggle_btn.disabled = mode != DJ_Controller.E_Performance_Pad_Mode.BEAT_JUMP
...
func _on_jump_beat_jump_toggled(enabled: bool) -> void:
    if _syncing_ui or not enabled:
        return
