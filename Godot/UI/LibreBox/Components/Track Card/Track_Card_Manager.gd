...
func Refresh_Details() -> bool:
    ...
    Artist_Name_Label.text = Utility.Return_Valid(
        Song_Resource.Main_Artist.Artist_Name if Song_Resource.Main_Artist else null, tr("KEY_UNTITLED")
    )
    Album_Name_Label.text = Utility.Return_Valid(
        Song_Resource.Song_Album.Album_Name if Song_Resource.Song_Album else null, tr("KEY_UNTITLED")
    )
...
func _clear_card_visuals() -> void:
    if Track_Genres_Containers:
        for genre_child in Track_Genres_Containers.get_children():
            genre_child.free()
    Track_Name_Label.text = tr("KEY_NO_TRACK")
